...
变量qlWhiteList，在V2P里面添加，格式：
xxx&yyy
xxx和yyy是要导出的变量名，多变量间用&连起来，支持正则
例如用ks或者ks.*可以导出所有名字中间带ks的变量
用.*可以导出所有变量
需要同步的时候，运行下这个脚本即可
*/

const jsname = 'V2P获取京东CK同步到青龙'
...
let qlWhiteList = $.getdata('qlWhiteList')
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if(!qlWhiteList) {
            console.log('未填写要导出的变量名，如果要导出所有变量，设置qlWhiteList为.*')
            return;
        }
        ...
        await searchEnv();
        await $.wait(10);
        await $exec('ls /usr/local/app/script/Store',{async cb(data, error){error ? console.error(error) : await getAllEnv(data)}})
    }
})()
.catch((e) => $.logErr(e))
.finally(() => $.done())

async function getAllEnv(data) {
    for(let name of data.split('\n')) {
        for(let whiteItem of qlWhiteList.split('&')) {
            if(name.search(whiteItem) > -1) {
                ...
                if(value) {
                    ...
                    for(let item of qlEnv) {
                        if(item.name == name) {
                            await $.wait(10);
                            await updateEnv(name,value,`V2P_sync@${(new Date()).getTime()}`,item._id);
                            isFound = true
                            break;
                        }
                    }
                    if(!isFound) {
                        await $.wait(10);
                        await addEnv(name,value,`V2P_sync@${(new Date()).getTime()}`);
                    }
                }
                break;
            }
...
async function GetRewrite() {
    if($request.url.indexOf(`api.m.jd.com/client.action`) > -1) {
        ...
        if(cookie.indexOf('pt_pin=') == -1 || cookie.indexOf('pt_key=') == -1) return;
        let pt_pin = cookie.match(/(pt_pin=.+?;)/)[1]
        let pt_key = cookie.match(/(pt_key=.+?;)/)[1]
        let ck = pt_pin + pt_key
        console.log(`获取到${pt_pin}的京东ck`)

        await getToken();
        if(!qlAuth) return;
        await $.wait(100);
        await searchEnv(pt_pin);

        let isFound = false
        for(let item of qlEnv) {
            if(item.name == 'JD_COOKIE' && item.value.indexOf(pt_pin) > -1) {
                await $.wait(100);
                await updateEnv('JD_COOKIE',ck,`V2P_sync@${(new Date()).getTime()}`,item._id);
                await $.wait(100);
        ...
        if(!isFound) {
            await $.wait(100);
            await addEnv('JD_COOKIE',ck,`V2P_sync@${(new Date()).getTime()}`);
        }
    }
}
